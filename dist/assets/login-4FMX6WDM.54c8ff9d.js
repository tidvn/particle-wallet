import{r as te,g as re}from"./unicode.f652bd91.js";import{K as ye}from"./chunk-6Y72V72Q.d3ce60d4.js";import{t as ve,i as Ae,H as we,o as Ee,E as Ce,T as ke,U as Ie}from"./chunk-EGL2DVBU.6bae04af.js";import{v as ie,$ as le,T as Ne,G as Se,l as Te}from"./chunk-IKGID5DT.12097f68.js";import{x as me}from"./chunk-4SMRMVPX.9956fd00.js";import{i as ue}from"./chunk-QNAXMJIS.3ce008d2.js";import{N as Me}from"./chunk-AMOVWUCP.62a50458.js";import{a1 as de,a9 as he,a2 as a,aa as r,ab as Be,ac as ge,ad as Fe,a3 as e,ae as ze,af as Ve,ag as Ke,ah as xe,ai as O,aj as He,ak as ce,al as oe,am as se,an as fe,ao as je,ap as pe}from"./index.fbf0f355.js";import{B as Le}from"./index.4896fbb2.js";import{t as De}from"./throttle.73ef307f.js";import{u as W}from"./useTranslation.88a96d8a.js";import{s as Re}from"./index.665683b7.js";import{F as U,v as be}from"./index.338a4e26.js";import{c as Ue}from"./DownOutlined.38f2da23.js";import"./index.9705f4c9.js";import"./colors.5dd5d985.js";import"./TextArea.4a47a176.js";var ae="data:image/png;base64,",Xe=`.captcha-box {
  text-align: center;
}
.captcha-box .error-tip {
  position: absolute;
  left: calc((100% - 316px) / 2);
  text-align: left;
  margin-top: 5px;
  font-weight: 400;
  font-size: 12px;
  line-height: 19px;
  color: var(--error-color);
  width: 100%;
}
.captcha-box .get-btn {
  width: 316px;
  padding: 0;
  margin-top: 14px;
  border: none;
  font-weight: 400;
  font-size: 16px;
  line-height: 19px;
  color: var(--text-color);
  background-color: transparent !important;
  box-shadow: none;
}
.captcha-box .get-btn[disabled] {
  font-weight: 400 !important;
  color: var(--secondary-text-color);
  opacity: 1 !important;
}
@media (max-width: 380px) {
  .captcha-box .get-btn {
    width: 90vw;
  }
}
@media (min-width: 600px) {
  .captcha-box .get-btn {
    width: calc(90 * var(--vw));
  }
}
.captcha-box .submit-btn {
  width: 322px;
  margin-top: 38px;
}
.captcha-box .btn-box {
  display: flex;
  flex-direction: column;
  align-items: center;
}
.captcha-box .back-box {
  position: relative;
  left: -4px;
  display: flex;
  justify-content: center;
  align-items: center;
  height: 19px;
  margin-top: 60px;
  margin-bottom: 30px;
  font-weight: 400;
  font-size: 16px;
  line-height: 19px;
  color: var(--accent-color);
  cursor: pointer;
  gap: 8px;
}
.captcha-box .back-box img {
  width: 9.42px;
  margin-right: 7px;
}
.captcha-box .back-box svg {
  position: relative;
  top: -1px;
  color: var(--accent-color);
}
.captcha-box .captcha-title {
  position: absolute;
  top: 14px;
  left: 0;
  right: 0;
  font-weight: 600;
  font-size: 18px;
  text-align: center;
  color: var(--text-color);
}
.captcha-box .captcha-form {
  padding-top: 44px;
  max-width: 316px;
  margin: auto;
}
.captcha-box .captcha-form .input-code-content {
  position: relative;
}
`;function Ye(K){let{sendCaptcha:h,setShowInputCaptcha:k,account:v,loginWithAccount:l,chain:w}=K,[x,b]=a.exports.useState(!1),{themeType:y,language:S}=xe(),[A,g]=a.exports.useState(""),{loginHandler:V}=he(),[p,c]=a.exports.useState(!1),[m,u]=a.exports.useState(!1),[C,E]=a.exports.useState(!1),[B,f]=a.exports.useState(!1),[F,H]=a.exports.useState(!1),{authCoreModal:j}=de(),{t:I}=W(),X=me(),Q=a.exports.useRef(null),T=/^\d{6}$/,[R,L]=a.exports.useState(1e3),[z,o]=a.exports.useState(60);Le(()=>{if(z>0){let t=z-1;o(t),t===0&&L(void 0)}},R);let i=()=>{o(60),L(1e3)},s=()=>{v.includes("@")?M({email:v}):fe({theme:y,language:S,getContainer:()=>j.rootBody}).then(t=>{M({phone:v,cf_turnstile_response:t})}).catch(()=>{X.error(I("error.server_20112"))})},M=t=>{h(t).then(()=>{i(),H(!1),f(!1)}).catch(()=>{o(0),L(void 0)})},N=t=>{u(!1);let d=t,D=/^[0-9]+$/g;d.length===0?(u(!1),c(!0)):D.test(d)?(c(!1),u(!1)):(c(!1),u(!0))},Y=t=>{c(!1),u(!1),E(!1),t=t||A,!(p||m)&&(T.test(t)?G(t):((t.length>0&&t.length<6||!T.test(t))&&(u(!0),c(!1)),t.length===0&&(u(!1),c(!0))))},_=async t=>{if(B)return;b(!0);let d;v.includes("@")?d={code:t,email:v,chain:w}:d={code:t,phone:v,chain:w};try{await V(d,!1)}catch(D){Z(D)}},Z=t=>{if(b(!1),je(t)){let d=t.error_code;d===pe.InvalidCode?E(!0):d===pe.ResendCode&&(f(!0),H(!0))}},$=t=>{N(t),g(t),c(!1),u(!1),E(!1),f(!1),T.test(t)&&G(t)},G=a.exports.useCallback(De(t=>{_(t)},1e3,{leading:!0,trailing:!1}),[]),P=()=>{var t;(t=document.getElementsByClassName("input-code-item")[Math.min(A.length,5)])==null||t.focus()},n=a.exports.useMemo(()=>{let t=[p?I("login.enter_captcha"):"",m?I("login.captcha_input_error"):"",C?I("login.invalid_code"):"",B?I("login.please_send_again"):""].map(d=>d.trim()).filter(d=>!!d).join(",");return t.includes(",")?t+".":t},[p,m,C,B]);return a.exports.useEffect(()=>{var t;let d=document.getElementsByClassName("react-input-code"),D=J=>J.preventDefault();return(t=d[0])==null||t.addEventListener("contextmenu",D),()=>{var J;(J=d[0])==null||J.removeEventListener("contextmenu",D)}},[]),e.createElement("div",{className:"captcha-box"},e.createElement("style",null,Xe),e.createElement("div",{className:"captcha-form"},e.createElement("div",{className:"input-code-content"},e.createElement("div",{onClick:P},e.createElement(ye,{containerClassName:"react-input-code",inputClassName:"input-code-item",allowedCharacters:"numeric",length:6,ref:Q,placeholder:" ",onChange:$})))),e.createElement("div",{className:"error-tip"},n),e.createElement("div",{className:"btn-box"},e.createElement(O,{type:"primary",htmlType:"submit",className:"primary-antd-btn submit-btn",onClick:()=>Y(),disabled:F,loading:x},I("common.confirm")),e.createElement(O,{className:"get-btn",disabled:z>0,type:"link",onClick:s},I("login.send_again")," ",z>0?`(${z}s)`:""),!l&&e.createElement("div",{className:"back-box",onClick:()=>{k(!1)}},e.createElement(ue,{className:"arrow1-icon",name:"arrow1_icon"}),e.createElement("span",null,I("login.back")))))}var Je=Ye,ne=`.login-style .form-item {
  margin-bottom: 0;
}
.login-header0 {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 26px;
  margin-top: 26px;
  font-weight: 600;
  font-size: 22px;
  line-height: 22px;
  text-align: center;
  color: var(--text-color);
}
.login-header0 img {
  position: absolute;
  left: 0;
  margin-left: 16px;
}
@media (max-height: 610px) {
  .login-header0 {
    margin-top: 18px;
  }
}
.login-des {
  width: 100%;
  height: 23px;
  margin-top: 7px;
  font-weight: 400;
  font-size: 16px;
  line-height: 19px;
  text-align: center;
  color: var(--secondary-text-color);
}
.form-box {
  display: flex;
  flex-direction: column;
  align-items: center;
  width: 100%;
  padding-top: 44px;
}
.form-box .ant-form-item-explain-error {
  position: absolute;
}
.form-box .form-item {
  display: flex;
  flex-direction: column;
  width: 100%;
  text-align: center;
}
.form-box .email-box {
  width: 316px;
  max-width: 90vw;
  height: 47px;
  line-height: 47px;
  display: flex;
  justify-content: center;
  align-items: center;
  background-color: var(--input-background-color);
  border-radius: var(--primary-btn-border-radius);
  font-size: 15px;
  border: 1px solid var(--input-background-color);
  overflow: hidden;
}
@media (min-width: 600px) {
  .form-box .email-box {
    max-width: calc(90 * var(--vw));
  }
}
.form-box .email-box[data-focus='true'] {
  border: 1px solid var(--accent-color);
}
.form-box .email-box .email-input {
  width: 316px;
  max-width: 90vw;
  height: 47px;
  padding: 4px 13px;
  border: 1px solid var(--input-background-color);
  border-radius: var(--primary-btn-border-radius);
  font-size: 15px;
  line-height: 47px;
  color: var(--text-color);
  background-color: var(--input-background-color);
}
@media (min-width: 600px) {
  .form-box .email-box .email-input {
    max-width: calc(90 * var(--vw));
  }
}
.form-box .phone-box {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 316px;
  max-width: 90vw;
  height: 47px;
  border: 1px solid var(--input-background-color);
  border-radius: var(--primary-btn-border-radius);
  overflow: hidden;
  font-size: 15px;
  line-height: 47px;
  background-color: var(--input-background-color);
}
.form-box .phone-box[data-focus='true'] {
  border: 1px solid var(--accent-color);
}
.form-box .phone-box .phone-select {
  display: flex;
  flex-shrink: 0;
  justify-content: center;
  align-items: center;
  width: 60px;
  height: 47px;
  padding-left: 8px;
  font-weight: 500;
  font-size: 15px;
  color: var(--text-color);
  cursor: pointer;
  border-right: 1px solid var(--input-border-color);
}
.form-box .phone-box .phone-select .down-more {
  margin-left: 8px;
  font-size: 10px;
}
.form-box .phone-box .country-code {
  display: flex;
  flex-shrink: 0;
  align-items: center;
  height: 47px;
  margin: 0 8px;
  line-height: 47px;
  color: var(--text-color);
}
.form-box .phone-box .phone-input {
  flex-shrink: 1;
  width: 100%;
  height: 47px;
  padding: 0;
  border: none !important;
  border-radius: 0;
  outline: none;
  font-size: 15px;
  line-height: 47px;
  color: var(--text-color);
  background-color: transparent;
  box-shadow: none;
}
.form-box .phone-box .phone-input:focus {
  border: none !important;
  box-shadow: none !important;
}
@media (min-width: 600px) {
  .form-box .phone-box {
    max-width: calc(90 * var(--vw));
  }
}
.form-box .phone-input,
.form-box .email-input {
  height: 37px;
  line-height: 37px;
  width: 85%;
  overflow: hidden;
  border-color: var(--input-background-color) !important;
}
.form-box .phone-input:-webkit-autofill,
.form-box .email-input:-webkit-autofill,
.form-box .phone-input:-webkit-autofill:hover,
.form-box .email-input:-webkit-autofill:hover,
.form-box .phone-input:-webkit-autofill:focus,
.form-box .email-input:-webkit-autofill:focus {
  -webkit-text-fill-color: var(--text-color) !important;
  -webkit-box-shadow: 0 0 0 150px var(--input-background-color) inset !important;
}
.form-box .phone-input::-ms-reveal,
.form-box .email-input::-ms-reveal,
.form-box .phone-input::-ms-clear,
.form-box .email-input::-ms-clear {
  display: none;
}
.form-box .phone-input:focus,
.form-box .email-input:focus {
  color: var(--text-color, #000) !important;
  border-color: var(--input-background-color) !important;
}
.form-box .error-tip {
  position: absolute;
  margin-top: 51px;
  font-weight: 400;
  font-size: 12px;
  line-height: 19px;
  color: var(--error-color);
  width: 316px;
  left: calc((100% - 316px) / 2);
  text-align: left;
}
.form-box .submit-btn {
  width: 316px;
  max-width: 90vw;
  margin-top: 35px;
}
@media (min-width: 600px) {
  .form-box .submit-btn {
    max-width: calc(90 * var(--vw));
  }
}
@media (max-height: 610px) {
  .form-box {
    padding-top: 20px;
  }
}
.login-or {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 13px;
  margin: 35px 0;
  font-weight: 500;
  font-size: 11px;
  line-height: 22px;
  white-space: nowrap;
  color: var(--secondary-text-color);
  gap: 15px;
}
.login-or .or-line {
  width: 143px;
  border-top: 1px solid var(--icon-border-color);
}
@media (max-height: 610px) {
  .login-or {
    margin: 18px 0;
  }
}
.change-login-mode {
  display: flex;
  justify-content: center;
  width: 100%;
}
.change-login-mode .change-social {
  display: flex;
  flex-wrap: wrap;
  justify-content: center;
  width: 315px;
  height: 106px;
}
.change-login-mode .change-social .first-way {
  margin: 5px 10px;
}
.change-login-mode .change-social img {
  width: 43px;
  height: 43px;
  margin: 5px 10px;
  border: 1px solid var(--icon-border-color);
  border-radius: 50%;
  background: #fff;
  cursor: pointer;
}
.change-login-mode .change-social img:hover {
  opacity: var(--hover-opacity);
}
.change-login-mode .change-social .collapse-login-icon {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 43px;
  height: 43px;
  margin: 5px 10px;
  border-radius: 50%;
  font-size: 24px;
  color: grey;
  transform: rotate(180deg);
  cursor: pointer;
}
.change-login-mode .change-social .collapse-login-icon[data-is-collapse='false'] {
  transform: rotate(0deg);
}
.change-login-mode .change-social .collapse-login-icon svg {
  width: 43px;
  height: 43px;
  color: #b4b6c6;
}
.change-login-mode .change-social .collapse-login-icon svg circle {
  fill: var(--bg-color-2);
}
.change-login-mode .change-social .collapse-login-icon:hover {
  opacity: var(--hover-opacity);
}
.select-country {
  position: absolute;
  z-index: 1000;
  width: 316px;
  max-width: 90vw;
  height: 291px;
  margin-top: 96px;
  left: 50%;
  transform: translateX(-50%);
  margin-left: 0;
  border: 0;
  border: 1px solid var(--input-background-color);
  border-radius: var(--card-border-radius);
  overflow: auto;
  overflow-x: hidden;
  background-color: var(--input-background-color);
}
.select-country::-webkit-scrollbar {
  display: none;
  width: 0;
}
.select-country .country-item {
  display: flex;
  justify-content: space-between;
  align-items: center;
  width: 100%;
  height: 46px;
  border-bottom: 1px solid var(--card-unclickable-background-color);
  cursor: pointer;
}
.select-country .country-item:hover {
  background-color: var(--card-unclickable-background-color);
}
.select-country .country-item .country-box {
  display: flex;
  align-items: center;
}
.select-country .country-item .country-name {
  font-weight: 500;
  font-size: 14px;
  color: var(--text-color);
}
.select-country .country-item .country-code {
  margin-right: 15px;
  font-weight: 500;
  font-size: 14px;
  color: var(--secondary-text-color);
}
.select-country .country-item .country-flag {
  margin-right: 10px;
  margin-left: 15px;
}
@media (min-width: 600px) {
  .select-country {
    max-width: calc(90 * var(--vw));
  }
}
`;function Qe(K){let{t:h}=W(),{sendCaptcha:k,setShowInputCaptcha:v,account:l,supportAuthTypes:w,loginWithAccount:x}=K,[b]=U.useForm(),[y,S]=a.exports.useState(!1),[A,g]=a.exports.useState(!1),[V,p]=a.exports.useState(!1),c=a.exports.useRef(null),m=a.exports.useRef(null);a.exports.useEffect(()=>{x&&l&&k&&(b.setFieldsValue({email:l}),u(l))},[]);let u=E=>{m.current.focus(),E?ge.test(E)?C(E):S(!0):(g(!0),S(!1))},C=E=>{p(!0),k({email:E}).then(B=>{v(B)}).catch(()=>{p(!1)})};return e.createElement(e.Fragment,null,e.createElement("style",null,ne),e.createElement("div",{className:"login-style"},e.createElement(U,{name:"login-form",onFinish:E=>u(E.email.trim()),className:"form-box",requiredMark:!1,form:b,labelCol:{style:{textAlign:"left"}}},e.createElement("div",{className:"email-box",ref:c},e.createElement(U.Item,{name:"email",label:"",className:"form-item",initialValue:l!=null&&l.includes("@")?l:""},e.createElement(be,{className:"email-input",placeholder:h("login.email_address")||void 0,ref:m,onChange:()=>{S(!1),g(!1)},readOnly:x,onFocus:()=>{c.current.setAttribute("data-focus","true")},onBlur:()=>{c.current.removeAttribute("data-focus")}}))),e.createElement("div",{className:"error-tip"},y&&h("login.email_format_error")),e.createElement("div",{className:"error-tip"},A&&h("login.input_email_holder")),e.createElement(U.Item,{className:"form-item"},e.createElement(O,{type:"primary",htmlType:"submit",className:"primary-antd-btn submit-btn",loading:V},h("login.get_captcha"))),!x&&w.length>0&&e.createElement("div",{className:"login-or"},e.createElement("div",{className:"or-line"}),h("login.or"),e.createElement("div",{className:"or-line"})))))}var Ze=Qe;function Ge(K){let{t:h}=W(),{sendCaptcha:k,setShowInputCaptcha:v,account:l,supportAuthTypes:w,loginWithAccount:x}=K,[b,y]=a.exports.useState(!1),{themeType:S,language:A}=xe(),[g,V]=a.exports.useState(["United States","us","1"]),[p]=U.useForm(),c=a.exports.useRef(),{authCoreModal:m}=He(),[u,C]=a.exports.useState(!1),[E,B]=a.exports.useState(!1),[f,F]=a.exports.useState(!1),H=me(),j=a.exports.useRef(null),[I,X]=a.exports.useState();a.exports.useEffect(()=>{if(b)return document.addEventListener("click",Q,!0),()=>{document.removeEventListener("click",Q,!0)}},[b]),a.exports.useEffect(()=>{let o;if(l&&!l.includes("@")&&ce(l)){let i=oe(l),s=i.countryCallingCode.toString(),M=i.country;if(p.setFieldsValue({phone:i.nationalNumber}),s&&M){let N=te.filter(Y=>Y[2].toString()===s&&Y[1].toLowerCase()===M.toLowerCase());N&&N.length>0&&(o=N[N.length-1])}}if(!o){let i=(navigator.languages&&navigator.languages.length?navigator.languages[0]:navigator.language).split("-"),s=i.length>1?i[1]:i[0];s&&s.length>0&&(o=te.find(M=>M[1].toUpperCase()===s))}o&&V(o),x&&l&&k&&ce(l)&&R(l,o?o[1].toUpperCase():void 0)},[]);let Q=o=>{setTimeout(()=>{var i;(i=c.current)!=null&&i.contains(o.target)||y(!1)})},T=(o,i,s=!0)=>{if(!o&&s){B(!0);return}if(o?.length===1){C(!0);return}return se(o,i)?(C(!1),!0):(C(!0),!1)},R=(o,i)=>{if(j.current.focus(),!o){B(!0);return}if(o.length===1){C(!0);return}if(i||(i=g[1].toUpperCase()),se(o,i)){let s=i.toUpperCase(),M=oe(o,s).format("E.164");X(M),F(!0),fe({theme:S,language:A,getContainer:()=>m.rootBody}).then(N=>{L(N,M)}).catch(()=>{F(!1),H.error(h("error.server_20112"))})}else C(!0)},L=(o,i)=>{F(!0),k({phone:i,cf_turnstile_response:o}).then(s=>{v(s)}).catch(()=>{F(!1)})},z=()=>{try{if(l!=null&&l.includes("@"))return"";if(l?.length>0)return oe(l).nationalNumber}catch{}return""};return e.createElement(e.Fragment,null,e.createElement("style",null,ne),e.createElement("div",{className:"login-style"},b&&e.createElement("div",{className:"select-country",ref:c},te.map((o,i)=>e.createElement("div",{key:i,className:"country-item",onClick:()=>{V(o),y(!1),setTimeout(()=>{let s=p.getFieldsValue().phone;s&&T(s,o[1].toUpperCase(),!1)})}},e.createElement("div",{className:"country-box"},e.createElement("div",{className:"country-flag"},re(`${o[1]}`)),e.createElement("span",{className:"country-name"},o[0])),e.createElement("div",{className:"country-code"},"+"+o[2])))),e.createElement(U,{name:"login-form",onFinish:o=>R(o.phone,void 0),className:"form-box",requiredMark:!1,form:p,labelCol:{style:{textAlign:"left"}}},e.createElement("div",{className:"phone-box"},e.createElement("div",{className:"phone-select",defaultValue:`+${g[2]}`,onClick:()=>{f||y(!0)}},re(`${g[1]}`),e.createElement(Ue,{className:"down-more"})),e.createElement("div",{className:"country-code"},"+",g[2]),e.createElement(U.Item,{name:"phone",label:"",className:"form-item",getValueFromEvent:o=>o.target.value.replace(/\D+/g,""),initialValue:z()},e.createElement(be,{type:"text",className:"phone-input",placeholder:h("login.mobile_number")||void 0,ref:j,onChange:()=>{C(!1),B(!1)},readOnly:x,onFocus:()=>{var o;(o=document.querySelector(".phone-box"))==null||o.setAttribute("data-focus","true")},onBlur:()=>{var o;(o=document.querySelector(".phone-box"))==null||o.removeAttribute("data-focus")}}))),e.createElement("div",{className:"error-tip"},u&&h("login.phone_format_error")),e.createElement("div",{className:"error-tip"},E&&h("login.input_phone_holder")),e.createElement(U.Item,{className:"form-item"},e.createElement(O,{type:"primary",htmlType:"submit",className:"primary-antd-btn submit-btn",loading:f},h("login.get_captcha")))),!x&&w.length>0&&e.createElement("div",{className:"login-or"},e.createElement("div",{className:"or-line"}),h("login.or"),e.createElement("div",{className:"or-line"}))))}var Pe=Ge,qe="data:image/png;base64,",Oe=K=>{let{supportAuthTypes:h,preferredAuthType:k,onPreferredAuthTypeChange:v,thirdpartyAuthLogin:l}=K,[w,x]=a.exports.useState(!0),b=g=>{l(g)},y=g=>{v?.(g)},S={email:ie,phone:le,facebook:Ne,google:Se,apple:ve,twitter:Te,discord:Ae,github:we,twitch:Ee,microsoft:Ce,linkedin:ke,jwt:Ie,passkeys:qe},A=a.exports.useMemo(()=>{let g=h,V=g.includes(r.email)&&g.includes(r.phone),p=[];V&&(k===r.phone?p.push(e.createElement("img",{key:"email-login",src:ie,alt:"",onClick:()=>y(r.email)})):p.push(e.createElement("img",{key:"phone-login",src:le,alt:"",onClick:()=>y(r.phone)})));let c=g.filter(u=>u!==r.email&&u!==r.phone);p.push(...c.map(u=>e.createElement("img",{key:`${u}-login`,src:S[u],alt:"",onClick:()=>b(u)})));let m=e.createElement("div",{key:"collapse-login",className:"collapse-login-icon",onClick:()=>x(!w),"data-is-collapse":w},e.createElement(ue,{className:"collapse-icon",name:"collapse_icon"}));return p.length>5&&p.splice(4,0,m),w&&p.splice(5,p.length-5),p},[w,k,h]);return e.createElement("div",{className:"change-login-mode","data-collapse":!!w},e.createElement("div",{className:"change-social"},A))},We=Oe,_e=`.login-container-box {
  position: relative;
  z-index: 1;
  width: 100%;
  height: 100%;
  overflow: hidden;
  display: flex;
  flex-direction: column;
}
.login-container-box .wrap {
  position: relative;
  width: 100%;
  height: 100%;
  padding-bottom: 40px;
  overflow-y: auto;
  padding-top: 56px;
  flex: 1;
}
@media (max-width: 600px) {
  .login-container-box .wrap {
    padding-top: 7vh;
  }
}
.login-container-box .wrap .login-logo {
  display: flex;
  justify-content: center;
  align-items: center;
  height: 64px;
  line-height: 1.5715;
}
.login-container-box .wrap .login-logo .product-logo {
  max-width: 100%;
  height: 64px;
  object-fit: contain;
}
.login-container-box .wrap .placeholder-wrap {
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 100%;
}
.login-container-box .footer-box {
  position: absolute;
  bottom: 10px;
  margin: 0;
  text-align: center;
}
@media only screen and (max-height: 300px) {
  .login-container-box .footer-box {
    position: absolute;
    top: unset;
    bottom: 10px;
    margin-bottom: 0;
  }
}
[data-login-type='facebook'][data-hash='#/login'] .footer-box,
[data-login-type='google'][data-hash='#/login'] .footer-box {
  position: absolute !important;
}
`;function $e(K){var h,k,v,l,w;let{email:x,phone:b,code:y,jwt:S,socialType:A,nonce:g,prompt:V,authorization:p,chain:c}=K||{},{modalOptions:m,setLoginAuthorization:u}=de(),{loginHandler:C,socialAuthLogin:E}=he(),B=()=>{var n,t,d,D,J;let q=(t=(n=m.authTypes)==null?void 0:n.indexOf(r.phone))!=null?t:-1,ee=(D=(d=m.authTypes)==null?void 0:d.indexOf(r.email))!=null?D:-1;return A||(!Ve(b)&&(q>=0||!((J=m.authTypes)!=null&&J.length))||!x&&(q>=0&&ee<0||q>=0&&ee>=0&&q<ee)?r.phone:r.email)},[f,F]=a.exports.useState(B()),{t:H}=W(),[j,I]=a.exports.useState(""),[X,Q]=a.exports.useState(!1),[T,R]=a.exports.useState(!1),[L,z]=a.exports.useState(m.authTypes||[]),o=n=>{Q(n)};a.exports.useEffect(()=>{document.documentElement.setAttribute("data-login-type",f||"")},[f]),a.exports.useEffect(()=>{if(T)z([]);else{let n=Object.values(r).filter(t=>t!==r.jwt);if(!m.authTypes)z(n);else{let t=m.authTypes.filter(d=>d!==r.jwt&&n.includes(d));A&&!t.includes(A)&&t.push(A),!t.includes(r.email)&&!t.includes(r.phone)&&t.length>0&&t.unshift(r.email),t.length===1&&(t[0]===r.email||t[0]===r.phone)&&(t.length=0),z(t)}}},[T,m.authTypes]),a.exports.useEffect(()=>{u(p?{authorization:p,chain:Be(c)}:void 0),M()},[]);let i=async n=>(I("email"in n&&n.email?n.email:n.phone),await Ke(n).then(t=>t)),s=()=>y&&y.match(/^\d{6}$/),M=()=>{if(x&&f===r.email)ge.test(x)&&(s()?N({email:x,code:y,chain:c}):(R(!0),I(x)));else if(b&&f===r.phone){let n=Fe(b);n&&(s()?N({phone:n,code:y,chain:c}):(R(!0),I(n)))}else S?(F(r.jwt),R(!0),N({jwt:S,chain:c})):A&&(y&&g?N({socialType:A,code:y,nonce:g,chain:c}):Y(A))},N=a.exports.useCallback(n=>{C(n)},[C]),Y=n=>{F(n),E({socialType:n,prompt:V,authorization:p,chain:c})},_=()=>{if(X)return e.createElement(Je,{sendCaptcha:i,setShowInputCaptcha:o,account:j,loginWithAccount:T,chain:c});if(f===r.email&&!s())return e.createElement(Ze,{sendCaptcha:i,setShowInputCaptcha:o,setLoginType:Z,account:x||j,supportAuthTypes:L,loginWithAccount:!!x||T});if(f===r.phone&&!s())return e.createElement(Pe,{sendCaptcha:i,setShowInputCaptcha:o,setLoginType:Z,account:b||j,supportAuthTypes:L,loginWithAccount:!!b||T})},Z=n=>{F(n)},$=n=>n===r.jwt?"JWT":n.titleCase(),G=n=>n===r.jwt?H("login.create_wallet"):H("login.social_login").format($(n)),P=a.exports.useMemo(()=>{var n;let t=(n=m.customStyle)==null?void 0:n.logo;return t||(t=ae),t||""},[(h=m.customStyle)==null?void 0:h.logo]);return e.createElement("div",{className:"login-container-box"},e.createElement("style",null,ne),e.createElement("style",null,_e),e.createElement("div",{className:"wrap"},e.createElement("div",{className:"login-logo"},e.createElement(Re,{className:"product-logo",preview:!1,height:64,src:P,key:P,placeholder:e.createElement("div",{className:"placeholder-wrap"},e.createElement(ze,{style:{fontSize:24},spin:!0})),fallback:ae,onError:n=>{n.currentTarget.src=ae}})),e.createElement("div",{className:"login-header0"},(v=(k=m.customStyle)==null?void 0:k.projectName)!=null?v:H("common.particle_auth")),X?e.createElement("div",{className:"login-des"},j):e.createElement("div",{className:"login-des"},(f===r.email||f===r.phone)&&!s()?(w=(l=m.customStyle)==null?void 0:l.subtitle)!=null?w:H("login.to_continue").format("App"):e.createElement("div",{className:"third-party-des"},G(f))),_(),!X&&!T&&(f===r.email||f===r.phone)&&!s()&&e.createElement(We,{supportAuthTypes:L,preferredAuthType:f,onPreferredAuthTypeChange:F,thirdpartyAuthLogin:Y})),e.createElement(Me,{className:"footer-box-v2"}))}var ft=$e;export{ft as default};
