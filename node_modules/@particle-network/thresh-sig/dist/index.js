...
__export(src_exports, {
  ...
  get_message_to_sign: () => get_message_to_sign,
  initWasm: () => initWasm
});
...
var Ecdsa = class {
  ...
  async signTronTx(endpoint, authParams, txData) {
    ...
    return `0x${import_buffer.Buffer.from(JSON.stringify(tx)).toString("hex")}`;
  }
  async rpcRequest(endpoint, authParams, chainId, method, ...params) {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        Authorization: `Basic ${import_buffer.Buffer.from(`${authParams.project_uuid}:${authParams.project_client_key}`, "utf8").toString("base64")}`
      },
      body: JSON.stringify({
        id: +new Date(),
        jsonrpc: "2.0",
...
function get_message_to_sign(str) {
  ...
  if (isLegacyTx(txData)) {
    ...
    const tx = import_tx.Transaction.fromTxData(txData, {
      common: import_common.Common.custom({ chainId }, { hardfork: import_common.Hardfork.SpuriousDragon })
    });
    return tx.getMessageToSign();
  } else {
    const tx = import_tx.TransactionFactory.fromTxData(txData);
    return tx.getMessageToSign();
  }
}
...
async function __wbg_init(input) {
  ...
  if (typeof input === "undefined") {
    input = new URL("../wasm/thresh_sig_wasm_bg.wasm", import_meta.url);
  }
...
var EdDsa = class {
  ...
  async _sign(endpoint, authParams, message) {
    ...
    const RBuf = import_buffer3.Buffer.from(sign.R.bytes_str.padStart(POINT_BYTES_SIZE * 2, "0"), "hex");
    const sBuf = import_buffer3.Buffer.from(sign.s.padStart(SCALAR_BYTES_SIZE * 2, "0"), "hex");
    const sBufLE = toLittleEndian(sBuf);
    return import_buffer3.Buffer.concat([RBuf, sBufLE], POINT_BYTES_SIZE + SCALAR_BYTES_SIZE);
  }
};
